...
import { Search, Filter, Phone, Mail, MapPin, AlertCircle, X, Building, Plus, Edit, Trash2, Check, User, FileText, Package, Calendar, DollarSign, CheckCircle, Clock } from 'lucide-react';
...
const ClientesPage: React.FC<ClientesPageProps> = ({
  ...
  const [clienteIdEdicao, setClienteIdEdicao] = useState<string | null>(null);
  ...
  const handleEdit = async (cliente: Cliente) => {
    ...
    setClienteIdEdicao(cliente.id); // ✅ NOVO: Definir ID para carregar devoluções
  ...
  const resetForm = async () => {
    ...
    setClienteIdEdicao(null); // ✅ NOVO: Limpar ID de edição
  ...
  // ✅ NOVO: Carregar devoluções quando a aba for ativada e houver clienteIdEdicao
  useEffect(() => {
    if (activeTab === 'devolucoes' && clienteIdEdicao) {
      carregarDevolucoes(clienteIdEdicao);
    }
  }, [activeTab, clienteIdEdicao]);
                  ...
                  <input type="hidden" value={formData.empresa_id} />
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        ...
                        {clienteIdEdicao && (
                          <button
                            onClick={() => carregarDevolucoes(clienteIdEdicao)}
